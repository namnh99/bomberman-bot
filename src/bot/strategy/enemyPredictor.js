...
/**
 * Find best position to bomb based on predicted enemy locations
 * Returns bombing position with highest hit probability
 */
export function findPredictiveBombPosition(
  ...
  for (let dy = -futureTicks; dy <= futureTicks; dy++) {
    for (let dx = -futureTicks; dx <= futureTicks; dx++) {
      ...
      if (moveDistance > futureTicks) continue

      const hitScore = calculatePredictiveHitScore(
        bombX,
        bombY,
        enemyPredictions,
        explosionRange,
  ...
  candidates.sort((a, b) => b.hitScore - a.hitScore)
  return candidates[0] || null
}

/**
 * Calculate probability of hitting enemies at predicted positions
 */
function calculatePredictiveHitScore(bombX, bombY, enemyPredictions, range, bombDelay) {
  let totalScore = 0

  for (const prediction of enemyPredictions) {
    for (const predicted of prediction.predictedPositions) {
      // Check if enemy will be in blast radius when bomb explodes
      if (predicted.steps < bombDelay || predicted.steps > bombDelay + 3) continue

      const distance = manhattanDistance(bombX, bombY, predicted.x, predicted.y)

      if (distance <= range) {
        // In blast radius
        totalScore += predicted.probability * (1 - distance / (range + 1))
      }
    }
  }
